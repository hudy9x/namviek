...
export const mdDBoardGetComponents = async (dboardId: string) => {
  return dashboardComponentModel.find({ dashboardId: dboardId })
}

export const mdDBoardDelComponent = async (componentId: string) => {
  return dashboardComponentModel.findByIdAndDelete(componentId)
}
...
export const mdDBoardAddComponent = async (
  data: Omit<IDashboardComponentField, 'id'>
) => {
  return dashboardComponentModel.create(data)
}
...
export const mdDBoardQueryBurnChart = async (
  ...
  const tasks = await taskModel
    .find(where)
    .select('id dueDate assigneeIds plannedDueDate')
    .lean()
    .catch(err => {
  ...
  const dates = handleDates(tasks as TaskChart[])
  const ideal = handleIdeal(dates, tasks as TaskChart[], type)
  const actual = handleActual(dates, tasks as TaskChart[], type)
...
export const mdDBoardQueryColumn = async ({
  ...
  const tasks = await taskModel
    .find(where)
    .select('id assigneeIds taskStatusId projectId taskPoint dueDate')
    .lean()
  ...
  if (tasks.length) {
    for (let i = 0; i < tasks.length; i++) {
      const task = tasks[i]
      const colName = task.assigneeIds[0].toString()
      ...
      const type = task.taskStatusId.toString()
