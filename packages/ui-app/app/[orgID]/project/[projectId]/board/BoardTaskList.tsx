...
export const BoardTaskList = ({ groupId, items }: IBoardTaskListProps) => {
  ...
  return (
    <Droppable droppableId={groupId} type="task">
      {(provided, snapshot) => (
        <div
          className={`border-2 pt-2 ${snapshot.isDraggingOver
          ...
          <Scrollbar style={{ height: 'calc(100vh - 83px - 38px - 85px)' }}>
            <div className="space-y-3 px-3 pb-[30px]">
              {items.map((item, itemIndex) => {
                return <BoardCard key={item} index={itemIndex} id={item} />
              })}
