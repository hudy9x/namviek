...
export default function Badge({ title, color = 'gray' }: IBadeProps) {
  ...
  color === 'red' && classes.push('bg-red-300 text-gray-50')
  color === 'green' && classes.push('bg-green-500 text-gray-100')
  color === 'yellow' && classes.push('bg-yellow-500 text-gray-50')


  return <div className={`${classes.join(' ')} text-[10px] text-center rounded-sm px-1`}>
    {title}
  </div>
}
